...
public class Ex01_Car {
	// 클래스에 중괄호 사이 (블럭킹 내부)에 있는 모든것들은 해당하는 이름의 클래스 멤버이다.
	// 자동차의 속력, 최대속력, 색상, 제조사 등의 속성은 필드(변수)로 관리를 하고,
	// 자동차가 달라거나 멈추거나 하는 기능의 관리는 메소드로 한다.
	int speed; // 자동차의 속력 ( 기본값 0 )
	
	// 멤버의 구분 : 크게 2가지 !
	
	// ①인스턴스 멤버 (IM,Iv(인스턴스변수), im(인스턴스메소드).. 등등) : ※ new ※
	...
	// 단, ※ 메모리에 항상 먼저 올라가야 하기때문에 인스턴스 멤버에 바로 접근이 불가능함 => 인스턴스화 과정후 사용가능
