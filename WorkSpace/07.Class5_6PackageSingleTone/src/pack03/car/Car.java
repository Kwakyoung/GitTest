...
import pack01.hankok.Tire;

public class Car {
	// 객체지향 프로그래밍에서는 각 부분의(모듈) 기능을 따로 분리하여 클래스로 나눈다.
	// 필요할때 필요한 클래스를 불러와서 하나의 객체를 완성한다.
	// package : 동일한 이름의 클래스를 사용해도 구별할수있게 해주는 유일한 식별자
	// 같은 패키지 내부에서는 똑같은 이름의 클래스(객체)가 존재 할 수 없음.
	
	// 인터넷 주소의(도메인) 반대로 사용하는 네이밍을 가지고있음.
	// com.naver.www <= 유일하게 식별 할 수 있게 패키지 이름을 지음.
